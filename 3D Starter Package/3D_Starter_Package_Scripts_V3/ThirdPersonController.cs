...
namespace DigitalWorlds.StarterPackage3D
{
    ...
    public class ThirdPersonController : PlayerMovementBase
    {
        ...
        [Tooltip("The input action used for looking. Set to mouse movement by default.")]
        [SerializeField] private InputAction lookAction = new("Look", InputActionType.Value, "<Mouse>/delta");
        ...
        [Tooltip("The input action used for sprinting. Set to left shift by default.")]
        [SerializeField] private InputAction sprintAction = new("Sprint", InputActionType.Button, "<Keyboard>/leftShift");
        ...
        [SerializeField] private InputAction jumpAction = new("Jump", InputActionType.Button, "<Keyboard>/space");
        ...
        public void AdjustJumps(int adjustment)
        {
            jumps += adjustment;
        }
        ...
        private void HandleInput()
        {
            ...
            moveInput = cameraTransform.forward * axis.y + cameraTransform.right * axis.x;
            moveInput.y = 0;
            ...
            if (moveInput.sqrMagnitude > 0.01f) // Prevents rotating when input is very low
            {
                ...
                if (animator != null)
                {
                    animator.SetBool(animationParameters.isRunningBool, true);
                }
            }
            else
            {
                ...
                if (animator != null)
                {
                    animator.SetBool(animationParameters.isRunningBool, false);
                }
            }
        ...
        private void Movement()
        {
            ...
            if (isGrounded)
            {
                ...
                {
                    OnLandEvent(rb.linearVelocity.y);
                }
            ...
            if (canMove)
            {
                if (isGrounded && sprintAction.IsPressed())
                {
        ...
        private void HandleCamera()
        {
            if (!lockCamera)
            {
                Vector2 lookInput = lookAction.ReadValue<Vector2>() * 0.1f;
